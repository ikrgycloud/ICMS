...
export function StudentCoursesView() {
  ...
  async function load() {
    ...
    try {
      ...
      const nextCatalogSemesters = Array.from(
        new Set<number>((next?.catalog || []).map((course: any) => Number(course.semester || 0)).filter(Boolean)),
  ...
  const semesterOptions = useMemo(
    () => [...new Set<number>((data?.filters?.available_semesters || []).map((value: number) => Number(value)))].sort((left, right) => left - right),
    [data],
  )
  const catalogSemesters = useMemo(
    () => [...new Set<number>((data?.catalog || []).map((course: any) => Number(course.semester || 0)).filter(Boolean))].sort((left, right) => left - right),
    [data],
  )
...
export function StudentAttendanceView() {
  ...
  const student = data?.student || {}
  const semesterOptions = useMemo(
    () => [...new Set<number>((data?.filters?.available_semesters || []).map((value: number) => Number(value)).filter(Boolean))].sort((left, right) => right - left),
    [data],
  )
