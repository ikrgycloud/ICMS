...
export default function FacultyHome({ user, go }: { user: any; go: (v: string) => void }) {
  ...
  return <main className="prof-overview fade-in">
    ...
    <section className="prof-stat-grid">{stats.map(([label, value], i) => <article className={`prof-stat p${i} ${i === 1 ? 'prof-stat-action' : ''}`} key={String(label)} onClick={i === 1 ? () => setShowSections(true) : undefined} role={i === 1 ? 'button' : undefined} tabIndex={i === 1 ? 0 : undefined} onKeyDown={i === 1 ? event => { if (event.key === 'Enter' || event.key === ' ') setShowSections(true) } : undefined}><span>{statIcons[i]}</span><div><b>{value}</b><small>{label}{i === 1 ? ' · View details' : ''}</small></div></article>)}</section>
    ...
    {showSections && <Modal title="My Sections" onClose={() => setShowSections(false)} className="prof-sections-modal" footer={<button className="btn btn-out" onClick={() => setShowSections(false)} type="button">Close</button>}>
      <p className="prof-sections-modal-intro">Sections assigned to you by the Academic Office.</p>
      <div className="prof-table-wrap"><table className="prof-table"><thead><tr><th>Course</th><th>Term</th><th>Section</th><th>Schedule</th><th>Room</th><th>Students</th><th>Attendance Avg</th></tr></thead><tbody>
        {sections.map((section: any) => <tr key={section.id}><td><b>{section.course_code}</b><br />{section.title}</td><td>{section.term || `Semester ${section.course_semester || '—'}`}</td><td>{section.section}</td><td>{section.schedule || 'TBD'}</td><td>{section.room || 'TBD'}</td><td>{section.enrolled || 0}</td><td><span className="prof-progress"><i style={{ width: `${section.attendance_pct || 0}%` }} /></span>{section.attendance_pct == null ? '—' : `${Math.round(section.attendance_pct)}%`}</td></tr>)}
        {!sections.length && <tr><td colSpan={7}>No sections assigned.</td></tr>}
      </tbody></table></div>
    </Modal>}
  </main>
}
