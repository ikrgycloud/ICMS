...
import AcademicCatalog from './modules/AcademicCatalog'
...
import BranchManagement from './modules/BranchManagement'
...
const CHAIRMAN_DISPLAY: Record<string, { label: string; group: string }> = {
  ...
  hr: { label: 'New Institute Creation', group: 'Strategy & Insights' },
...
const PRINCIPAL_NAV = [
  ...
  ['Academic / People', 'Academic Setup', 'academic_catalog'], ['Academic / People', 'Examinations', 'examinations'],
...
export default function App({ onLogout }: { onLogout: () => void }) {
  ...
  useEffect(() => {
    ...
    const principalVirtualModule = user?.office_n === 4 && ['faculty_staff', 'curriculum', 'courses_subjects', 'facilities', 'at_risk_students', 'compliance', 'approval_history', 'escalations', 'leave', 'recruitment', 'permissions', 'academic_catalog'].includes(view)
  ...
  const principalGroups = PRINCIPAL_NAV.reduce((out: Record<string, any[]>, [group, label, key]) => {
    const source = displayModules.find((module: any) => String(module.key) === key)
...
function ModuleView({ view, module, user, onChange, go }: any) {
  ...
  switch (view) {
    ...
    case 'examinations':
      ...
      return user.office_n === 4 ? <PrincipalExaminations /> : <Examinations caps={caps} />
    case 'scores':
    ...
    case 'hr':
      return user.office_n === 1 ? <BranchManagement /> : <HR caps={caps} />
    case 'faculty_staff':
      return <FacultyStaff />
    case 'leave':
    ...
    case 'approvals':
      return user.office_n === 1
        ...
        : user.office_n === 3
          ? <CampusHeadApprovals />
        : <Workflows user={user} onChange={onChange} mode="action" />
    case 'workflows':
      return <Workflows user={user} onChange={onChange} mode="monitor" />
