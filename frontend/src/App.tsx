...
import Transport, { TransportOverview } from './modules/Transport'
...
export default function App({ onLogout }: { onLogout: () => void }) {
  ...
  useEffect(() => {
    ...
    const parentTransportModule = user?.persona === 'parent' && view === 'transport'
    const transportOverviewModule = user?.office_n === 31 && view === 'overview'
    if (transportOverviewModule) {
      setView('transport')
      return
    }
    if (!principalVirtualModule && !parentTransportModule && !ws.modules.some((module: any) => module.key === view)) {
  ...
  const displayModules = useMemo(
    () => [...rawModules, ...(user?.persona === 'parent' && !rawModules.some((module: any) => module.key === 'transport') ? [{ key: 'transport', label: 'Transport', group: 'Services', enabled: true }] : [])]
      .filter((module: any) => !(user?.persona === 'student' && module.key === 'students'))
      .filter((module: any) => !(user?.office_n === 31 && module.key === 'overview'))
  ...
  const hiddenSidebarGroups = new Set(['Academics', 'Authority', 'Reference'])
  const hiddenSidebarKeys = new Set(['calendar', 'academic_calendar'])
  const groupKeys = [...order.filter(key => groups[key] && !hiddenSidebarGroups.has(key)), ...Object.keys(groups).filter(key => !order.includes(key) && !hiddenSidebarGroups.has(key))]
  const current = displayModules.find((module: any) => module.key === view) || displayModules[0]
  const transportOfficeSidebar = user.office_n === 31
  const principalGroups = PRINCIPAL_NAV.reduce((out: Record<string, any[]>, [group, label, key]) => {
    ...
    if (hiddenSidebarGroups.has(group)) return out
    if (transportOfficeSidebar && ['academic_calendar', 'calendar', 'directory', 'matrices'].includes(key)) return out
    if (!hiddenSidebarGroups.has(group)) ;(out[group] = out[group] || []).push({ key, label, group, source, enabled: Boolean(source) })
    return out
  }, {})
        ...
        <nav className="side-nav">
          {(principalShell ? Object.keys(principalGroups) : facultyShell ? Object.keys(facultyGroups) : groupKeys).map(group => (
            <div key={group}>
              ...
              {(principalShell ? principalGroups[group] : facultyShell ? facultyGroups[group] : groups[group]).filter((module: any) => !hiddenSidebarKeys.has(module.key)).map((module: any) => (
                <button
...
function ModuleView({ view, module, user, onChange, go }: any) {
  ...
  switch (view) {
    case 'overview':
      ...
      if (user.office_n === 31) return <TransportOverview caps={caps} />
