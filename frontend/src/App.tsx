...
import FacultySections from './personas/FacultySections'
...
import { FacultyMaterials, StudentMaterials } from './personas/CourseMaterials'
...
const FACULTY_NAV = [
  ...
  ['Teaching & Academics', 'Course Materials', 'course_materials'], ['Teaching & Academics', 'Academic Calendar', 'academic_calendar'],
  ['Students & Mentoring', 'My Students', 'students'], ['Students & Mentoring', 'Mentoring / Advisees', 'mentoring'],
  ['Research', 'Research & Guidance', 'research'],
  ['Self Service', 'My Profile', 'directory'], ['Self Service', 'Leave & Requests', 'workflows'], ['Self Service', 'Payroll', 'hr'],
  ['Workflow', 'My Requests', 'workflows'], ['Workflow', 'My Approvals', 'approvals'],
] as const

const ASSOCIATE_PROFESSOR_NAV = [
  ['Workspace', 'Overview', 'overview'], ['Workspace', 'My Schedule', 'my_schedule'], ['Workspace', 'Messages', 'messages'],
  ['Teaching & Academics', 'My Courses & Sections', 'academics'], ['Teaching & Academics', 'Attendance', 'attendance'],
  ['Teaching & Academics', 'Assessments & Marks', 'assessments'], ['Teaching & Academics', 'Examinations', 'examinations'],
  ['Teaching & Academics', 'Course Materials', 'course_materials'], ['Teaching & Academics', 'Academic Calendar', 'academic_calendar'],
  ['Communication', 'Announcements', 'announcements'],
  ['Self Service', 'My Profile', 'directory'], ['Self Service', 'Leave & Requests', 'workflows'], ['Self Service', 'Payroll', 'payroll'],
...
const FACULTY_ACTIVE_LABEL: Record<string, string> = {
  ...
  academics: 'My Sections', attendance: 'Attendance', assessments: 'Assessments', marks_entry: 'Marks Entry', examinations: 'Examinations',
  research: 'Research & Guidance', academic_calendar: 'Academic Calendar', directory: 'My Profile',
  students: 'My Students', mentoring: 'Mentoring / Advisees', hr: 'Payroll', approvals: 'My Approvals',
}
...
export default function App({ onLogout }: { onLogout: () => void }) {
  ...
  useEffect(() => {
    ...
    const facultyVirtualModule = user?.persona === 'faculty' && ['assessments', 'marks_entry', 'examinations_manage', 'course_materials', 'students', 'mentoring', 'hr', 'approvals', 'coordination', 'academic_risk', 'course_registrations', 'messages', 'announcements', 'payroll', 'research'].includes(view)
    if (!principalVirtualModule && !facultyVirtualModule && !ws.modules.some((module: any) => module.key === view)) {
  ...
  const facultyNav = user.office_n === 12
    ? [...ASSOCIATE_PROFESSOR_NAV,
      ...(ws.faculty_features?.course_coordination ? [['Coordination', 'Course Coordination', 'coordination']] : []),
      ...(ws.faculty_features?.advisees ? [['Students & Advising', 'My Advisees', 'mentoring'], ['Students & Advising', 'Academic Risk', 'academic_risk']] : []),
      ...(ws.faculty_features?.course_registrations ? [['Students & Advising', 'Course Registrations', 'course_registrations']] : []),
      ...(ws.faculty_features?.research ? [['Research', 'Research & Publications', 'research']] : []),
    ] as any[]
    : FACULTY_NAV as any[]
  const facultyGroups = facultyNav.reduce((out: Record<string, any[]>, [group, label, key]) => {
    const source = displayModules.find((module: any) => module.key === key)
    // Professor navigation is always available. Individual screens and API
    // actions still enforce the user's actual role permissions.
...
function ModuleView({ view, module, user, onChange, go }: any) {
  ...
  switch (view) {
    ...
    case 'mentoring':
      if (user.persona === 'faculty') return <FacultyMentoring />
      return <Students caps={caps} />
    case 'coordination':
    ...
    case 'academics':
      if (user.persona === 'faculty') return <FacultySections go={go} />
    ...
    case 'assessments':
      if (user.persona === 'faculty') return <FacultyAssessments go={go} />
      return <Examinations caps={caps} />
    case 'marks_entry':
      if (user.persona === 'faculty') return <FacultyMarks go={go} />
      return <Examinations caps={caps} />
    case 'course_materials':
      if (user.persona === 'faculty') return <FacultyMaterials />
      if (user.persona === 'student') return <StudentMaterials />
      return <div className="empty">Course materials are not available for this role.</div>
    case 'examinations_manage':
      return <Examinations caps={caps} />
    case 'scores':
