...
export default function CampusEscalations() {
  ...
  const [selected, setSelected] = useState<any>(null), [events, setEvents] = useState<any[]>([]), [reason, setReason] = useState(''), [error, setError] = useState(''), [message, setMessage] = useState('')
  ...
  async function openEscalation(row: any) {
    try {
      const response = await api.escalation(row.id)
      setSelected(response.escalation || row)
      setEvents(response.events || [])
    } catch (err: any) { setError(err?.message || 'Unable to load escalation details.') }
  }
  ...
  return <div className="fade-in campus-head-page campus-workspace escalation-workspace">
    ...
    {selected && <div className="modal-bg" onClick={() => setSelected(null)}><div className="modal risk-modal escalation-detail-modal" onClick={event => event.stopPropagation()}><div className="modal-h"><div><span className="modal-kicker">Escalation record</span><h3>Escalation detail</h3><p>{selected.destination} · {selected.priority}</p></div><button className="modal-x" type="button" onClick={() => setSelected(null)}>×</button></div><div className="modal-b risk-detail"><h4>Escalation Details</h4><div className="risk-detail-grid"><div><label>Status</label><strong><StatePill s={selected.status} /></strong></div><div><label>Priority</label><strong>{selected.priority}</strong></div><div><label>Source</label><strong>{selected.source_type} · {selected.source_ref}</strong></div><div><label>Reason</label><strong>{selected.reason || 'No reason recorded'}</strong></div></div><h4>Routing</h4><div className="risk-detail-grid"><div><label>Owner</label><strong>{selected.owner || 'Unassigned'}</strong></div><div><label>Destination</label><strong>{selected.destination}</strong></div><div><label>Campus</label><strong>{selected.campus_scope_id}</strong></div></div><h4>Timeline</h4><div className="escalation-timeline">{events.length ? events.map(event => <div className="escalation-timeline-item" key={event.id}><strong>{event.event_type}</strong><span>{event.previous_status || '—'} → {event.new_status || '—'}</span><small>{new Date(event.created_at).toLocaleString('en-IN')}{event.reason ? ` · ${event.reason}` : ''}</small></div>) : <p className="workflow-empty">No escalation events recorded.</p>}</div><textarea className="inp" rows={3} placeholder="Reason / follow-up note" value={reason} onChange={e => setReason(e.target.value)} /></div><div className="modal-f risk-modal-actions">{selected.status === 'DRAFT' && <button className="btn btn-crimson" type="button" onClick={() => change('submit')}>Submit</button>}{['SUBMITTED', 'RECEIVED'].includes(selected.status) && <button className="btn btn-out" type="button" onClick={() => change('follow_up')}>Follow up</button>}{!['RESOLVED', 'CLOSED'].includes(selected.status) && <button className="btn btn-crimson" type="button" onClick={() => change('resolve')}>Resolve</button>}{selected.status === 'RESOLVED' && <button className="btn btn-out" type="button" onClick={() => change('close')}>Close</button>}</div></div></div>}
  </div>
}
