...
export default function PrincipalCompliance({ go }: any) {
  ...
  const load = async (nextFilters = filters, resetScroll = false) => {
    ...
    setError(''); setLoading(true); setData((previous: any) => previous ? { ...previous, requirements: [] } : previous)
    try { const response = await api.complianceRequirements(nextFilters); if (version === requestVersion.current) setData(response) }
    catch (e: any) { if (version === requestVersion.current) setError(e.message || 'Unable to load accreditation and compliance data.') }
    finally { if (version === requestVersion.current) setLoading(false) }
  }
  useEffect(() => { window.scrollTo({ top: 0, behavior: 'auto' }); void load(emptyFilters) }, [])
  const updateFilter = (key: string, value: string) => { const next = { ...filters, [key]: value }; setFilters(next); void load(next, true) }
  if (!data && !error) return <Spinner />
  if (error) return <div className="empty-state"><h3>Unable to load accreditation and compliance data.</h3><p>{error}</p><button className="btn btn-crimson" onClick={() => void load()}>Retry</button></div>
  return <div className="fade-in principal-operations"><PageHead title="Accreditation & Compliance" sub="Operational compliance requirements for your authorized campus." />
    ...
    {selected && <RequirementModal detail={selected.requirement} onClose={() => setSelected(null)} onRefresh={() => { api.complianceRequirement(selected.requirement.id).then(setSelected); void load() }} go={go}/>}</div>
}
...
function RequirementModal({ detail, onClose, onRefresh, go }: any) {
  ...
  const action = async (kind: string) => {
    if ((kind === 'return' || kind === 'escalate') && !reason.trim()) { setError(`Please provide a reason before ${kind}ing this requirement.`); return }
    setBusy(kind); setError('')
    try { await api.decideWorkflow(detail.workflow_id, kind, reason); setReason(''); await onRefresh() }
    catch (e: any) { setError(e.message || `Unable to ${kind} this compliance requirement.`) }
    finally { setBusy('') }
  }
  const button = (kind: string, style: string) => actions.includes(kind) ? <button className={`btn ${style}`} disabled={!!busy} onClick={() => action(kind)}>{busy === kind ? 'Working...' : kind[0].toUpperCase() + kind.slice(1)}</button> : null
  return <Modal title="Compliance Requirement" className="principal-compliance-modal" onClose={onClose} footer={<>
    {actions.length ? <><span className="workflow-required">{detail.action_message || 'Decision required'}</span>{button('review', 'btn-teal')}{button('approve', 'btn-crimson')}{button('reject', 'btn-rose')}{button('return', 'btn-rose')}{button('escalate', 'btn-out')}</> : <span className="workflow-unavailable">{detail.action_message || 'Informational — no action required.'}</span>}
    <button className="btn btn-brass" onClick={() => { sessionStorage.setItem('workflow-open', detail.workflow_id); go?.('workflows') }}>View Workflow</button>
  </>}><div className="compliance-detail"><h3>{detail.title}</h3>{error && <div className="err-box">{error}</div>}<Info label="Reference" value={detail.reference_code}/><Info label="Category" value={detail.category}/><Info label="Responsible Department" value={detail.responsible_department}/><Info label="Campus" value={detail.campus}/><Info label="Priority" value={detail.priority}/><Info label="Due Date" value={detail.due_date || 'Not configured'}/><Info label="Current Status" value={detail.status}/><Info label="Evidence" value={detail.evidence_reference || 'Not available'}/><p>{detail.description}</p><h4>Decision History</h4>{detail.history.length ? detail.history.map((item: any) => <div className="snap" key={item.id}><span>{item.decision} · {item.stage}</span><b>{item.actor}</b><small>{item.reason || 'No remarks'} · {item.at}</small></div>) : <p className="principal-empty">No workflow decisions have been recorded.</p>}<label>Reason / Remarks<textarea className="inp" rows={4} value={reason} onChange={e => setReason(e.target.value)} placeholder="Required when returning or escalating; recorded in workflow history and audit."/></label></div></Modal>
}
