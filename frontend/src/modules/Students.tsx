...
export default function Students({ caps: _caps }: { caps?: any }) {
  const [q, setQ] = useState(""),
    ...
    [filters, setFilters] = useState<any>({
      ...emptyFilters,
      risk: sessionStorage.getItem("principal-student-risk") || "",
    }),
    [error, setError] = useState(""),
    [mode, setMode] = useState<"overview" | "list" | "analysis">("overview"),
    [profile, setProfile] = useState<any>(null),
    [full, setFull] = useState(false);
  const load = (query = q, department = dept, p = page, f = filters) => {
    setError("");
    return api
      .students(query, department, p, 25, f)
      .then(setData)
      .catch((e: any) => setError(e.message || "Unable to load students."));
  };
  useEffect(() => {
    sessionStorage.removeItem("principal-student-risk");
    load();
  }, []);
  ...
  const applyQuick = (risk: string) => {
    const next = { ...filters, risk };
    setFilters(next);
    setPage(1);
    setMode("list");
    load(q, dept, 1, next);
  ...
  if (!data)
    return error ? (
      <div className="empty-state">
        ...
        <p>{error}</p>
        <button className="btn btn-crimson" onClick={() => load()}>
          Retry
        </button>
      </div>
    ) : (
  ...
  return (
    <div className="fade-in students-ref">
      <PageHead
        title={
          mode === "overview"
            ? "Students — Overall Overview"
            : mode === "analysis"
              ? "Backlog Analysis"
              : "Students"
        }
        sub="Campus-scoped, read-only Principal oversight of student population, performance and academic risk."
      />
      <div className="students-context">
        Academic context and filter choices are loaded from student records
        within your authorized campus.
      </div>
      <div className="students-kpis">
        <Metric n={summary.all_students} t="Total Students" />
        <Metric n={summary.at_risk} t="At Risk" />
        <Metric
          n={
            summary.attendance_available
              ? summary.attendance_risk
              : "Unavailable"
          }
          t="Attendance Risk"
        />
        <Metric n={summary.academic_risk} t="Academic Risk" />
        <Metric
          n={
        ...
        />
        <Metric n={summary.average_cgpa ?? "Unavailable"} t="Average CGPA" />
      </div>
      <div className="student-quick-tabs">
        {[
          ["", "All Students"],
          ["at-risk", "At Risk"],
          ...
          <button
            key={t}
            className={filters.risk === v ? "active" : ""}
      ...
      <div className="students-filter">
        <label>
          ...
          >
            ...
            {options.academic_years.map((x: string) => (
        ...
        <label>
          ...
          >
            ...
            {options.programs.map(([id, name]: any) => (
        ...
        <label>
          ...
          >
            ...
            {options.departments.map(([code, name]: any) => (
        ...
        <label>
          ...
          <select
            ...
            onChange={(e) => {
              ...
              setFilters({ ...filters, studyYear: value, semester: "" });
            }}
          >
            <option value="">All Years</option>
            {options.study_years.map((x: number) => (
              <option key={x} value={x}>
                {x}
                {x === 1 ? "st" : x === 2 ? "nd" : x === 3 ? "rd" : "th"} Year
              </option>
            ))}
          </select>
        </label>
        <label>
          ...
          >
            ...
            {options.semesters.map((x: number) => (
              <option key={x} value={x}>
                Semester {x}
              </option>
            ))}
        ...
        />
        <button className="btn btn-crimson" onClick={apply}>
          Apply
        </button>
        <button
      ...
      {mode === "overview" ? (
        ...
        <Analysis s={summary} back={() => setMode("overview")} />
      ) : (
        <List
          rows={rows}
          data={data}
          ...
          setPage={setPage}
          open={(x: any) =>
            api.studentProfile(x.id).then((p) => {
              setProfile(p);
              setFull(false);
            })
          }
      ...
      {profile && (
        <Modal
          title={full ? "Student 360°" : "Student Profile"}
          onClose={() => setProfile(null)}
          footer={
            !full ? (
              <button className="btn btn-crimson" onClick={() => setFull(true)}>
                View Full Profile
              </button>
            ) : undefined
          }
        >
          <Profile data={profile} full={full} />
        </Modal>
      )}
    </div>
  );
