...
import { FiCalendar, FiChevronDown, FiChevronUp, FiEdit2, FiFilter, FiMapPin, FiPlus, FiSearch, FiTrash2, FiUser, FiX, FiPhone, FiClock, FiSend, FiCheckCircle, FiRadio, FiNavigation } from 'react-icons/fi'
import { FaBus } from 'react-icons/fa'

type Tab = 'dashboard' | 'requests' | 'routes' | 'vehicles' | 'drivers' | 'students' | 'allocations'
type ModalType = 'driver' | 'vehicle' | 'route' | 'stop' | 'student-assign' | 'request' | null
  ...
  const handleSave = async () => {
    try {
      ...
      if (modal === 'driver') {
        ...
        setSuccess(`Driver created. Login username: ${createdDriver.username} · Password: ${createdDriver.initial_password}`)
        ...
        <>
          <Kpis
            items={[
              ...
              { label: 'Allocated', value: data.allocations.length },
          ...
          <Section title="Pending Transport Requests">
            {pending.length > 0 ? (
              <RequestsTable rows={pending} routes={data.routes} onApprove={(r) => {
          ...
          <Section title="Vehicle Occupancy">
            <OccupancyCards vehicles={data.vehicles} />
          </Section>
        </>
      )}
      ...
      {tab === 'requests' && (
        <Section title="All Transport Requests">
          {data.requests.length > 0 ? (
            <RequestsTable rows={data.requests} routes={data.routes} onApprove={(r) => {
              setSelectedRequest(r)
              setModal('request')
            }} />
          ) : (
            <EmptyState text="No transport requests" />
          )}
        </Section>
      )}
      ...
      {tab === 'routes' && (
        ...
        >
          {data.routes.length > 0 ? (
                    ...
                    {expandedRoutes[route.id] && (
                      <div className="transport-route-body">
                        ...
                        {sortedStops.length > 0 ? (
                          ...
                          <p className="hint">No pickup points added</p>
                        )}
                      </div>
                    )}
                  </div>
            ...
            <EmptyState text="No routes created" />
          )}
        </Section>
      )}
      ...
      {tab === 'vehicles' && (
        ...
        >
          {data.vehicles.length > 0 ? (
            ...
            <EmptyState text="No vehicles added" />
          )}
        </Section>
      )}
      ...
      {tab === 'drivers' && (
        ...
        >
          {data.drivers.length > 0 ? (
            ...
            <EmptyState text="No drivers added" />
          )}
        </Section>
      )}
                    ...
                    <div className="student-transport-filters"><span className="student-filter-label"><FiFilter /> Filters</span>
                      ...
                      <div className="transport-filter">
                        ...
                        <select className="select" value={studentFilters.route} onChange={(e) => setStudentFilters({ ...studentFilters, route: e.target.value })}>
                          <option value="All">All</option>
                          {data.routes.filter((r: any) => r.status !== 'INACTIVE').map((r: any) => <option key={r.id} value={r.id}>{r.route_code ? `${r.route_code} · ` : ''}{r.name}</option>)}
                        </select>
                      </div>
                      <div className="transport-filter">
                          ...
                          >
                            <div className="student-vehicle-header-main">
                              ...
                              <div>
                                ...
                                <small>{vehicle?.vehicle_type || 'Bus'} · {allocations.length} {allocations.length === 1 ? 'student' : 'students'}</small>
                              </div>
                            </div>
                          ...
                          {expandedVehicles[vehicleId] && (
                            <div className="student-vehicle-body">
                              <div className="assigned-students-head">
                                ...
                                <button className="btn btn-sm btn-crimson" onClick={(e) => { e.stopPropagation(); const assignedRoute = data.routes.find((r: any) => r.status !== 'INACTIVE' && r.vehicle_no === vehicle.vehicle_number); setForm({ ...emptyForm, vehicle_id: vehicleId, route_id: assignedRoute?.id || '' }); setModal('student-assign') }}><FiPlus /> Add Student</button>
                              </div>
                              <div className="student-assignment-list">
                                {allocations.map((alloc: any) => (
                                  <div key={alloc.id} className="assigned-student">
                                    <div className="assigned-student-main">
                                      <span className="assigned-student-icon"><FiUser /></span>
                                      <div className="assigned-student-info">
                                        <div className="assigned-student-row">
                                          <div className="assigned-student-name-wrap">
                                            <strong>{alloc.student_name}</strong>
                                            <span className="mono">{alloc.student_id}</span>
                                          </div>
                                          <div className="assigned-student-dates">
                                            {alloc.assigned_at?.split('T')[0] || 'Current'}
                                          </div>
                                        </div>
                                        <div className="assigned-student-meta">
                                          <span><FiMapPin /> {alloc.pickup || 'Pickup stop'}</span>
                                          <div className="assigned-student-actions">
                                            <button className="student-icon-btn" title="Edit" onClick={() => { setSelectedAllocation(alloc); setForm({ ...emptyForm, student_id: alloc.student_id, route_id: alloc.route_id, pickup_stop_id: alloc.pickup_stop_id, drop_stop_id: alloc.drop_stop_id, vehicle_id: alloc.vehicle_id, driver_id: alloc.driver_id || '' }); setModal('student-assign') }}><FiEdit2 /></button>
                                            <button className="student-icon-btn danger" title="Delete" onClick={() => setDeleteConfirm({ type: 'allocation', id: alloc.id, name: `${alloc.student_name}'s assignment` })}><FiTrash2 /></button>
                                          </div>
                                        </div>
                                      </div>
                                    </div>
        ...
        <div className="modal-bg" onClick={() => setDeleteConfirm(null)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-h">
              ...
              <button className="modal-x" onClick={() => setDeleteConfirm(null)}>✕</button>
            </div>
          ...
          <div className="modal" onClick={(e) => e.stopPropagation()}>
                ...
                {modal === 'driver'
                  ...
                  : modal === 'vehicle'
                    ...
                    : modal === 'route'
                      ? 'Create Route'
                      : modal === 'stop'
                        ? `${selectedStop ? 'Edit stop' : 'Add stop'} · ${selectedRoute?.route_code}`
            ...
            <div className="modal-b">
              ...
              {modal === 'student-assign' && (
                <div className="grid-2">
                  <div className="form-row"><label>Student *</label><select className="select" value={form.student_id} onChange={(e) => setForm({ ...form, student_id: e.target.value })}><option value="">Select Student</option>{data.students?.map((s: any) => <option key={s.id} value={s.id}>{s.name} ({s.roll_no})</option>)}</select></div>
                  <div className="form-row"><label>Route *</label><select className="select" value={form.route_id} onChange={(e) => setForm({ ...form, route_id: e.target.value, pickup_stop_id: '', drop_stop_id: '' })}>
                  ...
                  <div className="form-row"><label>Driver (Optional)</label><select className="select" value={form.driver_id} onChange={(e) => setForm({ ...form, driver_id: e.target.value })}>
...
function DriverDashboard() {
  ...
  return (
    <div className="fade-in">
      ...
      <Kpis
        items={[
          { label: 'Vehicle', value: data.vehicle?.vehicle_number || '�' },
          { label: 'Capacity', value: data.vehicle?.capacity || '�' },
          { label: 'Route', value: data.route?.name || '�' },
            ...
            {trip ? (
              <>
                <div style={{ padding: '12px 14px', background: '#fbe9e7', borderRadius: 9, marginBottom: 16, color: '#c73532' }}>
                  ...
                  <p style={{ fontSize: 13, marginBottom: 4 }}>{data.route?.name} � {trip.trip_type}</p>
...
function OccupancyCards({ vehicles }: any) {
  return (
    <div className="grid-2">
      {vehicles.map((v: any) => (
