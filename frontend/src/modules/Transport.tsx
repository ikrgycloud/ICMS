...
export default function Transport({ caps }: { caps: any }) {
  const user = getUser() || {};
  const isDriver =
    Number(user.office_n) === 31 &&
    String(user.role || "")
      .toLowerCase()
      .includes("driver");

  if (["student", "parent"].includes(String(user.persona || "").toLowerCase()))
...
export function TransportOverview({
  ...
  const visibleRoutes = data.routes.filter((route: any) => {
    ...
    if (!text) return true;
    return [route.name, route.route_code, route.status]
      .join(" ")
      .toLowerCase()
      .includes(text);
  });
      ...
      {loading ? (
        ...
        <>
          <div className="kpi-row transport-clickable-kpis">
            {[
              ["Operational Routes", data.routes.length, "routes"],
              ["Vehicles", data.vehicles.length, "vehicles"],
              ["Drivers", data.drivers.length, "drivers"],
              ["Pending Requests", pending.length, "requests"],
              ["Allocated Students", data.allocations.length, "students"],
              ["Available Seats", seats, "vehicles"],
              ...
              <button
                ...
                aria-label={label}
                onClick={() => {
                  sessionStorage.setItem("transport-tab", target as string);
                  go?.("transport");
                }}
              >
                    ...
                    <div key={vehicle.id} className="transport-mini-row">
                      <div>
                        ...
                        <span>
                          ...
                          {Math.max(0, vehicle.capacity - vehicle.occupied)}{" "}
                          seats free
                        </span>
                      </div>
...
function StudentTransport() {
  ...
  const load = async () => {
    try {
      const [mine, reqs, routeData] = await Promise.all([
        api.myTransportAllocation(),
        api.transportRequests(),
        api.transportRoutes(),
      ]);
      setAllocation(mine?.allocation ?? (mine?.id ? mine : null));
      setRequests(reqs.requests || []);
      setRoutes(routeData.routes || []);
      setError("");
    } catch (e: any) {
      setError(e.message || "Unable to load transport assignments.");
    }
  };
  useEffect(() => {
    load();
  ...
  useEffect(() => {
    if (!allocation?.vehicle_id) return;
    const poll = () =>
      api
        .liveTransportLocation(allocation.vehicle_id)
  ...
  const submit = async () => {
    if (!routeId || !stopId)
      return setError("Please select a route and pickup point.");
    try {
      setBusy(true);
  ...
  if (allocation)
    return <StudentAssignedTransport allocation={allocation} routes={routes} />;
  return (
    <div className="student-transport-portal fade-in">
      <header className="student-portal-head">
        <div>
          <h1>My Transport</h1>
          <p>You currently don't have an active transport allocation.</p>
        </div>
      </header>
      <section className="student-portal-empty">
        <FaBus />
        <h2>You are not assigned to any bus.</h2>
        {error && <p className="notice rose">{error}</p>}
        {pending ? (
          <div className="student-pending">
            <FiCheckCircle /> Transport request pending
          </div>
        ) : (
          <>
            <p>
              Request a bus assignment by selecting your preferred route and
              pickup point.
            </p>
            <div className="student-request-form">
              <select
                className="select"
                value={routeId}
                onChange={(e) => {
              ...
              <button
                ...
                disabled={busy}
              >
                <FiSend /> {busy ? "Sending..." : "Request Bus Approval"}
              </button>
            </div>
          </>
        )}
...
function StudentAssignedTransport({
  allocation,
  routes,
}: {
  allocation: any;
  ...
  const stops =
    allocation.route_stops ||
    routes.find((r: any) => r.id === allocation.route_id)?.stops ||
    [];
  useEffect(() => {
    const poll = () =>
      api
  ...
  return (
    <div className="student-transport-portal fade-in">
      <header className="student-portal-head">
        <div>
          <h1>My Transport</h1>
          <p>Your active bus assignment and route details</p>
        </div>
      </header>
      <div className="student-portal-grid">
        <section className="student-portal-card student-portal-vehicle">
          <FaBus />
          <div>
            <span>Assigned Vehicle</span>
            <h2>{allocation.vehicle || "Vehicle unavailable"}</h2>
            <Pill
              s={allocation.vehicle_status || allocation.status || "ACTIVE"}
            />
          </div>
        </section>
        <section className="student-portal-card student-live-card">
          <h3>
            <FiRadio /> Live Bus Tracking{" "}
            {live?.status === "RUNNING" && (
              <span className="student-live-indicator">Live</span>
            )}
          </h3>
          {live?.status === "RUNNING" && live.location ? (
            <>
              <div className="student-live-map">
                <FiNavigation />
                <span>Current location received</span>
              </div>
              <div className="student-live-coordinates">
                {live.location.latitude.toFixed(5)},{" "}
                {live.location.longitude.toFixed(5)}
              </div>
              <p>
                Current / nearest stop: <b>{current?.name || "Calculating"}</b>
              </p>
              <p>
                Next stop: <b>{next?.name || "End of route"}</b>
              </p>
              {current?.distance_km != null && (
                <p>
                  Distance: <b>{current.distance_km.toFixed(2)} km</b>
                </p>
              )}
            </>
          ) : (
            <p>
              {live?.status === "ENDED"
                ? "Bus tracking has ended."
                : "Bus tracking has not started yet."}
            </p>
          )}
        </section>
        <section className="student-portal-card">
          ...
          <strong>{allocation.route || "Route unavailable"}</strong>
          <div className="student-route-stops">
            {stops.map((s: any) => (
              <div
                key={s.id}
                className={
                  s.id === allocation.pickup_stop_id
                    ? "pickup-stop selected"
                    : "pickup-stop"
                }
              >
                <span>{String(s.sequence).padStart(2, "0")}</span>
                <FiClock />
                <b>{s.pickup_time || "â€”"}</b>
                <label>{s.name}</label>
                {s.id === allocation.pickup_stop_id && <FiCheckCircle />}
              </div>
            ))}
          </div>
        </section>
        <section className="student-portal-card">
          ...
          <strong>{allocation.driver || "Not assigned"}</strong>
          {allocation.driver_phone && (
            <span>
              <FiPhone /> {allocation.driver_phone}
            </span>
          )}
        </section>
        <section className="student-portal-card student-pickup-card">
          <h3>Your Pickup Point</h3>
          <strong>
            <FiMapPin /> {allocation.pickup || "Not specified"}
          </strong>
          {allocation.pickup_time && (
            <span>
              <FiClock /> {allocation.pickup_time}
            </span>
          )}
        </section>
      </div>
...
function LiveBusLocation({ vehicleId }: { vehicleId: string }) {
  ...
  useEffect(() => {
    const poll = () =>
      api
        .liveTransportLocation(vehicleId)
        .then(setLive)
        .catch(() => setLive(null));
    poll();
    const timer = window.setInterval(poll, 10000);
  ...
  return (
    <section className="student-portal-card student-live-card">
      <h3>
        <FiRadio /> Live Bus Location
      </h3>
      {live?.status === "RUNNING" && live.location ? (
        <>
          <div className="student-live-map">
            <FiMapPin />
            <span>GPS location received</span>
          </div>
          <div className="student-live-coordinates">
            {live.location.latitude.toFixed(5)},{" "}
            {live.location.longitude.toFixed(5)}
          ...
          <small>
            Last updated{" "}
            {new Date(live.location.recorded_at).toLocaleTimeString()}
          </small>
        </>
      ) : (
        <p>
...
function TransportManager() {
  const [tab, setTab] = useState<Tab>(
    () => (sessionStorage.getItem("transport-tab") as Tab) || "dashboard",
  );
  useEffect(() => {
    sessionStorage.removeItem("transport-tab");
  }, []);
    ...
    try {
      ...
      if (type === "route") {
        ...
        if (!deleteConfirm.routeId) {
          throw new Error("Route is required to delete this pickup point");
        }

        await api.deleteTransportStop(
          deleteConfirm.routeId,
          id,
          deleteConfirm.name,
        );
  ...
  const handleSave = async () => {
    try {
      ...
      if (modal === "driver") {
        ...
        if (selectedDriver)
          await api.updateTransportDriver(selectedDriver.id, payload);
        else createdDriver = await api.createTransportDriver(payload);
        setSuccess(
          selectedDriver
            ? "Driver updated successfully"
            : `Driver created. Login: ${createdDriver?.login?.username || payload.phone} / demo123`,
        );
        ...
        if (selectedVehicle)
          await api.updateTransportVehicle(selectedVehicle.id, payload);
        else {
          const createdVehicle = await api.createTransportVehicle({
            ...payload,
            driver_id: null,
          });
          if (payload.driver_id)
            await api.updateTransportVehicle(createdVehicle.id, payload);
        }
        setSuccess(
          selectedVehicle
        ...
        if (selectedRoute)
          await api.updateTransportRoute(selectedRoute.id, payload);
  ...
  const filteredRequests = filterText
    ? data.requests.filter((row: any) =>
        [row.student_name, row.student_id, row.status]
          .join(" ")
          .toLowerCase()
          .includes(filterText),
      )
  ...
  const filteredRoutes = filterText
    ? data.routes.filter((row: any) =>
        [row.name, row.route_code, row.status]
          .join(" ")
          .toLowerCase()
          .includes(filterText),
      )
  ...
  const filteredVehicles = filterText
    ? data.vehicles.filter((row: any) =>
        [row.vehicle_number, row.vehicle_type, row.status]
          .join(" ")
          .toLowerCase()
          .includes(filterText),
      )
  ...
  const filteredDrivers = filterText
    ? data.drivers.filter((row: any) =>
        [row.name, row.employee_id, row.phone]
          .join(" ")
          .toLowerCase()
          .includes(filterText),
      )
    ...
    <div className="fade-in">
      ...
      {tab === "dashboard" && (
        <>
          <div className="kpi-row transport-clickable-kpis">
            {[
              ["Routes", data.routes.length, "routes"],
              ["Vehicles", data.vehicles.length, "vehicles"],
              ["Drivers", data.drivers.length, "drivers"],
              ["Allocated", data.allocations.length, "students"],
              ["Pending Requests", pending.length, "requests"],
              ["Available Seats", seats, "vehicles"],
            ].map(([label, value, target]) => (
              <button
          ...
          <Section title="Vehicle Occupancy">
            <OccupancyCards
              ...
              emptyText={
                filterText ? "No matching vehicles" : "No vehicles added"
              }
            />
          </Section>
        </>
      ...
      {tab === "routes" && (
        ...
        >
          {filteredRoutes.length > 0 ? (
            <div className="transport-routes-list">
              {filteredRoutes.map((route: any) => {
                const sortedStops = [...(route.stops || [])].sort(
                  (a: any, b: any) => Number(a.sequence) - Number(b.sequence),
                );
                return (
                  <div className="card transport-route-card" key={route.id}>
                    ...
                    >
                      ...
                      <div className="transport-route-right">
                        ...
                        <span className="transport-route-chevron">
                      ...
                      <div className="transport-route-body">
                        <div className="transport-route-body-header">
                          <div>
                            ...
                            <small>
                              Daily pickup schedule • ordered from first to last
                              stop
                            </small>
                          </div>
                          <button
                        ...
                        {sortedStops.length > 0 ? (
                          <div className="transport-stop-timeline">
                            {sortedStops.map((stop: any, index: number) => (
                              <div className="transport-stop" key={stop.id}>
                                <div className="transport-stop-rail">
                                  <div className="transport-stop-marker">
                                    {String(
                                ...
                                <div className="transport-stop-content">
                                  <div className="transport-stop-copy">
                                    <div className="transport-stop-time-inline">
                                      <span className="transport-stop-clock">
                                        {stop.pickup_time || "â€”"}
                                      </span>
                                      <span className="transport-stop-daily">
                                        Daily
                                      </span>
                                    </div>
                                  ...
                                  {stop.address && (
                                    <div className="transport-stop-address">
                                      {stop.address}
                                    </div>
                                  )}
                          ...
                          <div className="transport-stop-empty">
                            <span>
                              <FiMapPin />
                            </span>
                            <h4>No pickup points added</h4>
                            <p>
                              Add boarding points to define the pickup schedule
                              for this route.
                            </p>
                            <button
                              className="btn btn-sm btn-crimson"
            ...
            <EmptyState
              text={filterText ? "No matching routes" : "No routes created"}
            />
          )}
        </Section>
      )}
      ...
      {tab === "vehicles" && (
        ...
        >
          {filteredVehicles.length > 0 ? (
                  ...
                  <tbody>
                    {filteredVehicles.map((v: any) => (
                      <tr key={v.id}>
                        ...
                        <td className="transport-table-actions">
                          <button
            ...
            <EmptyState
              text={filterText ? "No matching vehicles" : "No vehicles added"}
            />
          )}
        </Section>
      )}
      ...
      {tab === "drivers" && (
        ...
        >
          {filteredDrivers.length > 0 ? (
            ...
            <EmptyState
              text={filterText ? "No matching drivers" : "No drivers added"}
            />
          )}
        </Section>
      )}
        ...
        <>
          {(() => {
            const filteredAllocations = (() => {
              ...
              if (studentFilters.student !== "All")
                result = result.filter(
                  (a: any) => a.student_id === studentFilters.student,
                );
              if (studentFilters.route !== "All")
                result = result.filter(
                  (a: any) => a.route_id === studentFilters.route,
                );
              if (studentFilters.stop !== "All")
                result = result.filter(
                  (a: any) => a.pickup_stop_id === studentFilters.stop,
                );
              return result;
            })();
            ...
            const getUniqueValues = (field: string) => {
              const values = new Set(
                data.allocations?.map((a: any) => a[field]).filter(Boolean),
              );
            ...
            return (
              <Section
                title={`Student Transport - ${filteredAllocations.length} ${filteredAllocations.length === 1 ? "record" : "records"}`}
                action={
                  <button
                    className="btn btn-sm btn-crimson"
                    onClick={() => setModal("vehicle")}
              ...
              >
                <div className="card">
                  ...
                  >
                    <div className="student-transport-search">
                      <FiSearch />
                      <input
                        type="text"
                    ...
                    <button
                      className="btn btn-sm btn-out transport-filter-toggle"
                      onClick={() => setShowStudentFilters((open) => !open)}
                    >
                      <FiFilter /> Filters{" "}
                      {showStudentFilters ? <FiChevronUp /> : <FiChevronDown />}
                    </button>
                    {showStudentFilters && (
                      <div className="student-transport-filters">
                        <div className="transport-filter">
                          ...
                          >
                            ...
                            {getUniqueValues("student_id").map((s: any) => (
                        ...
                        <div className="transport-filter">
                          ...
                          >
                            ...
                            {getUniqueValues("pickup_stop_id").map((s: any) => {
                              const stop = data.allocations.find(
                                (x: any) => x.pickup_stop_id === s,
                              );
                              return (
                                <option key={s} value={s}>
                                  {stop?.pickup || s}
                        ...
                        <div key={vehicleId} className="student-vehicle-group">
                          ...
                          >
                            <div className="student-vehicle-header-main">
                              <span className="student-route-badge">
                                <FaBus />
                              </span>
                              <div>
                                <div className="student-vehicle-name">
                                  {vehicle?.vehicle_number || "Vehicle"}
                                  <span className="student-vehicle-status">
                            ...
                            <div className="student-vehicle-header-side">
                              ...
                              <span className="student-vehicle-chevron">
                            ...
                            <div className="student-vehicle-body">
                              <div className="assigned-students-head">
                                ...
                                <button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    const assignedRoute = data.routes.find(
                                      (r: any) =>
                                        r.status !== "INACTIVE" &&
                                        r.vehicle_no === vehicle.vehicle_number,
                                    );
                                    setStudentAssignVehicleId(vehicleId);
                                    setForm({
                                      ...emptyForm,
                                  ...
                                  >
                                    <div className="student-info">
                                      ...
                                      <div className="student-details">
                                        <strong
                                          className="student-name student-detail-link"
                                          role="button"
                                          tabIndex={0}
                                          onClick={(e) => {
                                            e.stopPropagation();
                                        ...
                                        <span className="student-id">
                                          {alloc.roll_no || alloc.student_id}
                                        </span>
                                        <span className="pickup-point">
                                          <FiMapPin />{" "}
                                          {alloc.pickup || "Pickup stop"}
                                        </span>
                                      </div>
                                    </div>
                                    <div className="student-row-right">
                                      <span className="student-assigned-date">
                                        {alloc.assigned_at?.split("T")[0] ||
                                          "Current"}
                                      </span>
                                        ...
                                        <button
                                          ...
                                          onClick={(e) => {
                                            ...
                                            setForm({
                                              ...
                                              pickup_stop_id:
                                                alloc.pickup_stop_id,
                                              drop_stop_id: alloc.drop_stop_id,
                                        ...
                                        <button
                                          ...
                                          onClick={(e) => {
                                            ...
                                            setDeleteConfirm({
                                              type: "allocation",
                                              id: alloc.id,
                                              name: `${alloc.student_name}'s assignment`,
                                            });
                                          }}
                                        >
                                          <FiTrash2 />
            ...
            <div className="modal-b">
              <RequestApprovalForm
                ...
                onApprove={async (payload) => {
                  try {
                    await api.approveTransportRequest(
                      selectedRequest.id,
                      payload,
                    );
      ...
      {modal === "student-details" && selectedStudent && (
        <div className="modal-bg" onClick={closeModal}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-h">
              <h3>Student Transport Details</h3>
              <button
                className="modal-x"
                onClick={closeModal}
                aria-label="Close"
            ...
            <div className="modal-b">
              <div className="student-detail-hero">
                <span className="assigned-student-icon">
                  <FiUser />
                </span>
                <div>
                  <h3>{selectedStudent.student_name || "Student"}</h3>
                  <span className="mono">
                    Roll No:{" "}
                    {selectedStudent.roll_no || selectedStudent.student_id}
                  </span>
                </div>
              </div>
              <div className="student-detail-grid">
                <div>
                  <small>Route</small>
                  <b>
                    {selectedStudent.route || selectedStudent.route_id || "â€”"}
                  </b>
                </div>
                <div>
                  <small>Vehicle</small>
                ...
                <div>
                  ...
                  <b>
                    {selectedStudent.pickup ||
                      selectedStudent.pickup_stop_id ||
                      "â€”"}
                  </b>
                </div>
                <div>
                  <small>Status</small>
                  <Pill s={selectedStudent.status || "ACTIVE"} />
                </div>
              </div>
            </div>
            <div className="modal-f">
              <button className="btn btn-out" onClick={closeModal}>
                Close
              </button>
            </div>
          </div>
                    ...
                    >
                      ...
                      {data.vehicles.map((v: any) => {
                        const isAssigned =
                          ...
                          data.drivers.some(
                            (d: any) =>
                              d.vehicle_id === v.id && d.id !== form.driver_id,
                          );
                        return !isAssigned ? (
              ...
              {modal === "route" && (
                <div className="grid-2">
                  <FormField
                    ...
                    onChange={(v) => setForm({ ...form, route_code: v })}
                  />
                  <FormField
                    label="Route Name *"
                    value={form.name}
              ...
              {modal === "stop" && (
                <div className="grid-2">
                  <FormField
                    ...
                    onChange={(v) => setForm({ ...form, stop_name: v })}
                  />
                  <FormField
                    label="Sequence *"
                    type="number"
                  ...
                  <FormField
                    ...
                    onChange={(v) => setForm({ ...form, stop_address: v })}
                  />
                  <FormField
                    label="Pickup Time"
                    type="time"
                  ...
                  <FormField
                    ...
                    onChange={(v) => setForm({ ...form, drop_time: v })}
                  />
                </div>
              )}
                ...
                <div className="grid-2">
                  <div className="form-row">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setAllocationStudentSearch(e.target.value)
                      }
                    />
                    <select
                      className="select"
                    ...
                    >
                      ...
                      {data.students
                        ?.filter((s: any) =>
                          `${s.name} ${s.roll_no} ${s.id}`
                            .toLowerCase()
                            .includes(
                              allocationStudentSearch.trim().toLowerCase(),
                            ),
                        )
                        .map((s: any) => (
                          <option key={s.id} value={s.id}>
                  ...
                  <div className="form-row">
                    ...
                    >
                      ...
                      {data.vehicles.map((v: any) => (
                        <option key={v.id} value={v.id}>
                          {v.vehicle_number} (
                          {Math.max(0, v.capacity - v.occupied)} available)
                        </option>
                      ))}
                    </select>
                  </div>
    ...
    const id = navigator.geolocation?.watchPosition(
      (p) => {
        api
          ...
          .catch((e) =>
            setError(`GPS update failed: ${e.message || "Network error"}`),
          );
      },
      (e) => setError(`GPS unavailable: ${e.message}`),
          ...
          <div className="card-pad">
            {trip ? (
              ...
              <>
                ...
                >
                  Select trip type and start
                </p>
                <div style={{ display: "flex", gap: 10, marginBottom: 16 }}>
                  <button
                    className={
                      direction === "PICKUP" ? "btn btn-crimson" : "btn btn-out"
                    }
                  ...
                  <button
                    className={
                      direction === "DROP" ? "btn btn-crimson" : "btn btn-out"
                    }
    ...
    <div className="grid-2">
      {vehicles.map((v: any) => (
        <div className="card" key={v.id}>
          <div className="card-h">
            <h4 style={{ fontSize: 16, marginBottom: 0 }}>
              {v.vehicle_number}
            </h4>
            <Pill s={v.status} />
          </div>
          <div className="card-pad">
            ...
            <div className="fill-bar">
              <span
                style={{
                  width: `${Math.min(100, v.capacity ? (v.occupied / v.capacity) * 100 : 0)}%`,
                }}
              />
            </div>
          </div>
