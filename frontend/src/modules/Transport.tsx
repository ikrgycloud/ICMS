...
export function TransportOverview({ caps }: { caps: any }) {
    ...
    <div className="fade-in">
      ...
      {loading ? <Spinner /> : (
        <>
          <Kpis
            items={[
              { label: 'Operational Routes', value: data.routes.length },
              ...
              { label: 'Allocated Students', value: data.allocations.length },
                ...
                {data.vehicles.length > 0 ? data.vehicles.map((vehicle: any) => (
                  <div key={vehicle.id} className="transport-mini-row">
                    <div>
                      ...
                      <span>{vehicle.vehicle_type} · {Math.max(0, vehicle.capacity - vehicle.occupied)} seats free</span>
                    </div>
