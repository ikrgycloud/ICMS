...
function FeeSetup({ canManage, onOpenApprovals }: { canManage: boolean; onOpenApprovals: () => void }) {
  ...
  async function saveDraft() {
    ...
    try {
      setSaving(true)
      setError('')
      const payload = {...draft, effective_from: draft.effective_from || null, effective_to: draft.effective_to || null, lines: draft.lines.map((x:any) => ({...x, amount: Number(x.amount), due_date: x.due_date || null}))}
