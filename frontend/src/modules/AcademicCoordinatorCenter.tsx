...
const workflow = [
  ["Approved Curriculum", "approved"],
  ["Academic Calendar Draft", "draft"],
  ["Dean Academic Review", "review"],
  ["Vice Principal Operational Review", "review"],
  ["Publish Academic Calendar", "published"],
  ["Semester / Term Setup", "ready"],
];
const execution = [
  "Course Offerings",
  "HOD Inputs",
  "Approved Faculty Allocation",
  "Sections Ready",
  "Timetable Plan",
  "Conflict Engine",
  "Resolve / Replan",
  "HOD Review",
  "Vice Principal Timetable Approval",
  "Publish Timetable",
  "Faculty Portal + Student Portal",
  "Class Session Generated",
  "Faculty Self Check-in",
  "Student Attendance",
];
export default function AcademicCoordinatorCenter({
  onNavigate,
}: {
  ...
  const stats = useMemo(() => {
    ...
    return [
      [
        "Programs in Scope",
        new Set(courses.map((x: any) => x.program || x.dept)).size,
        "programs",
      ],
      ["Course Offerings", offerings.length, "offerings"],
      ["Sections Planned", offerings.reduce((n: number, x: any) => n + (x.section_readiness?.required_sections || 0), 0), "required sections"],
      ["Timetable Coverage", `${coverage}%`, "scheduled"],
      ["Active Conflicts", conflicts.filter((x: any) => !['Resolved', 'resolved'].includes(x.status)).length, "needs review"],
      [
        "Calendar Changes Pending",
        cal.filter((x: any) =>
          ["draft", "pending"].includes(String(x.status).toLowerCase()),
        ).length,
        "items",
      ],
      ["Curriculum Execution Issues", execution.reduce((n: number, x: any) => n + (x.execution_issues || []).filter((i: any) => i.status !== 'Resolved').length, 0), "open issues"],
      [
        "Needs My Action",
        offerings.filter((x: any) => !x.readiness?.ready).length + conflicts.filter((x: any) => !['Resolved', 'resolved'].includes(x.status)).length + execution.reduce((n: number, x: any) => n + (x.execution_issues || []).filter((i: any) => i.status !== 'Resolved').length, 0)
          ,
        "reviews",
      ],
    ];
  ...
  return (
    <div className="coordinator-center fade-in">
      <main className="coordinator-main">
        <PageHead
          title="Academic Coordinator Command Center"
          sub="Coordinate, review and monitor the complete academic operating cycle."
        />
        <div className="coordinator-banner">
          <div>
            <span>Operational readiness</span>
            <strong>
              {coverage === "100%" ? "Ready for delivery" : "In preparation"}
            </strong>
            <p>
              Live view across curriculum, calendar, sections, timetable and
              attendance.
            </p>
          </div>
          <div className="coordinator-banner-meta">
            <b>Office 17</b>
            <span>Coordination scope</span>
          </div>
        </div>
        <section className="coordinator-stat-grid">
        ...
        <section className="coordinator-card">
          <header>
            <div>
              <h2>Academic calendar workflow</h2>
              <p>
                Prepare and move the academic cycle through its approval
                hierarchy.
              </p>
            </div>
            <span className="coordinator-state">
              Coordinator prepares &amp; submits
            </span>
          </header>
          <div className="coordinator-flow">
            {workflow.map(([label, state], i) => (
              <div
                className={`coordinator-flow-step ${i < 1 ? "done" : i === 1 ? "current" : ""}`}
                key={label}
              >
                <i>{i + 1}</i>
                <strong>{label}</strong>
                <small>{state === "review" ? "Approval gate" : state}</small>
        ...
        <section className="coordinator-two-col">
          <article className="coordinator-card">
            <header>
              <div>
                <h2>Curriculum execution</h2>
                <p>
                  Approved curriculum → Curriculum Officer → Track execution
                </p>
              </div>
            </header>
            <div className="coordinator-readiness">
              ...
              <div>
                ...
                <b className={sections.length ? "ok" : "warn"}>
                  {sections.length ? "YES" : "NO"}
                </b>
              </div>
              <div>
                <span>Coverage on track?</span>
                <b className={coverage === "100%" ? "ok" : "warn"}>
                  {coverage === "100%" ? "YES" : "IN PROGRESS"}
                </b>
              </div>
            </div>
          ...
          <article className="coordinator-card">
            <header>
              <div>
                <h2>Timetable readiness</h2>
                <p>Scheduling checkpoint for academic delivery.</p>
              </div>
            </header>
            <div className="coordinator-checks">
              <div>
                <span>Sections with timetable</span>
                <b>
                  {sections.filter((x: any) => x.schedule && x.schedule !== "TBD").length} / {sections.length}
                </b>
              </div>
              <div>
        ...
        <section className="coordinator-card">
          <header>
            <div>
              <h2>End-to-end academic delivery</h2>
              <p>Operational hand-offs from offering to attendance.</p>
            </div>
          </header>
          <div className="coordinator-delivery-flow">
            {execution.map((item, i) => (
              <div
                className={i < 4 ? "ready" : i === 5 ? "attention" : ""}
                key={item}
              >
                <i>{i + 1}</i>
                <span>{item}</span>
