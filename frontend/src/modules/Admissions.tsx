...
export default function Admissions({ caps: capsProp, initialTab, sidebarNavigation = false, directorMode = false }: { caps?: PermissionCaps | null, initialTab?: string, sidebarNavigation?: boolean, directorMode?: boolean }) {
  ...
  const [binding, setBinding] = useState<any>({}), [search, setSearch] = useState('')
  const [eligibility, setEligibility] = useState<any[]>([]), [filters, setFilters] = useState<any>({ cycle_id: '', program_id: '', campus: '', status: '', quota_code: '', search: '' })
  const [rules, setRules] = useState<any[]>([]), [quotas, setQuotas] = useState<any[]>([]), [rule, setRule] = useState<any>(null), [quota, setQuota] = useState<any>(null), [eligibilityDetail, setEligibilityDetail] = useState<any>(null)
  const [phase4, setPhase4] = useState<any[]>([]), [seatPools, setSeatPools] = useState<any[]>([])
  const [counselling, setCounselling] = useState<any[]>([]), [assessment, setAssessment] = useState<any>(null), [counsellingItem, setCounsellingItem] = useState<any>(null)
  const [waitlist, setWaitlist] = useState<any[]>([]), [offers, setOffers] = useState<any[]>([]), [offerStatus, setOfferStatus] = useState('')
  const [programIntake, setProgramIntake] = useState<any[]>([]), [documentStatus, setDocumentStatus] = useState<any[]>([]), [phase5Status, setPhase5Status] = useState<any[]>([]), [finalApprovals, setFinalApprovals] = useState<any[]>([])
  const [directorData, setDirectorData] = useState<any>({ offers: [], pools: [], counselling: [], phase5: [] })
  const [seatPool, setSeatPool] = useState<any>(null)
  ...
  const loadRules = () => api.eligibilityRules(filters.cycle_id).then((x: any) => setRules(x.rules || [])).catch(e => setNotice({ outcome: 'DENY', reason: e.message }))
  const loadQuotas = () => api.eligibilityQuotas(filters.cycle_id).then((x: any) => setQuotas(x.quotas || [])).catch(e => setNotice({ outcome: 'DENY', reason: e.message }))
  const loadPhase4 = () => { api.assessmentQueue().then((x: any) => setPhase4(x.applications || [])).catch(e => setNotice({ outcome: 'DENY', reason: e.message })); api.admissionSeatPools(filters.cycle_id).then((x: any) => setSeatPools(x.seat_pools || [])).catch(e => setNotice({ outcome: 'DENY', reason: e.message })) }
  const loadCounselling = () => api.counsellingQueue({ cycle_id: filters.cycle_id, program_id: filters.program_id }).then((x: any) => setCounselling(x.applications || [])).catch(e => setNotice({ outcome: 'DENY', reason: e.message }))
  const loadWaitlist = () => api.admissionWaitlist().then((x: any) => setWaitlist(x.waitlist || [])).catch(e => setNotice({ outcome: 'DENY', reason: e.message }))
  const loadOffers = () => api.admissionOffers(offerStatus).then((x: any) => setOffers(x.offers || [])).catch(e => setNotice({ outcome: 'DENY', reason: e.message }))
  const loadProgramIntake = () => api.admissionProgramIntake().then((x: any) => setProgramIntake(x.program_intake || [])).catch(e => setNotice({ outcome: 'DENY', reason: e.message }))
  const loadDocumentStatus = () => api.admissionDocumentStatus().then((x: any) => setDocumentStatus(x.documents || [])).catch(e => setNotice({ outcome: 'DENY', reason: e.message }))
  ...
  const saveRule = () => act(async () => { const criteria = rule.rule_key === 'REQUIRED_DOCUMENT' ? { rule_type: rule.rule_key, document_type: rule.document_type } : { rule_type: rule.rule_key, field: rule.field, operator: rule.operator, value: rule.value }; const body = { cycle_id: rule.cycle_id, program_id: rule.program_id || null, quota_code: rule.quota_code, rule_key: rule.rule_key, criteria, active: rule.active }; const r = rule.id ? await api.updateEligibilityRule(rule.id, body) : await api.createEligibilityRule(body); setRule(null); return r }, loadRules)
  const saveQuota = () => act(async () => { const body = { ...quota, program_id: quota.program_id || null, priority: Number(quota.priority || 0) }; const r = quota.id ? await api.updateEligibilityQuota(quota.id, body) : await api.createEligibilityQuota(body); setQuota(null); return r }, loadQuotas)
  const openEligibilityDetail = (id: string) => api.eligibilityDetail(id).then(setEligibilityDetail).catch(e => setNotice({ outcome: 'DENY', reason: e.message }))
  const editRule = (r: any) => setRule({ ...r, field: r.criteria?.field || '', operator: r.criteria?.operator || '>=', value: r.criteria?.value ?? '', document_type: r.criteria?.document_type || r.criteria?.value || '' })
  if (!apps) return <Spinner />
  return <div className="fade-in"><PageHead title="Admissions" sub="Cycles, applications, review and eligibility" right={tab === 'cycles' && manage ? <button className="btn btn-brass" onClick={() => setCycle({ ...fresh })}>Create cycle</button> : undefined}/>
    {!sidebarNavigation && <div className="tabs"><button className={`tab ${tab === 'applications' ? 'on' : ''}`} onClick={() => setTab('applications')}>Applications</button><button className={`tab ${tab === 'review' ? 'on' : ''}`} onClick={() => setTab('review')}>Review Queue</button><button className={`tab ${tab === 'cycles' ? 'on' : ''}`} onClick={() => setTab('cycles')}>Admission Cycles</button>{caps.view_eligibility && <><button className={`tab ${tab === 'eligibility' ? 'on' : ''}`} onClick={() => setTab('eligibility')}>Eligibility</button><button className={`tab ${tab === 'rules' ? 'on' : ''}`} onClick={() => setTab('rules')}>Eligibility Rules</button><button className={`tab ${tab === 'quotas' ? 'on' : ''}`} onClick={() => setTab('quotas')}>Quotas</button><button className={`tab ${tab === 'decisions' ? 'on' : ''}`} onClick={() => setTab('decisions')}>Assessments / Allocation</button><button className={`tab ${tab === 'counselling' ? 'on' : ''}`} onClick={() => setTab('counselling')}>Counselling</button><button className={`tab ${tab === 'seatpools' ? 'on' : ''}`} onClick={() => setTab('seatpools')}>Seat Pools</button><button className={`tab ${tab === 'waitlist' ? 'on' : ''}`} onClick={() => setTab('waitlist')}>Waitlist</button><button className={`tab ${tab === 'offers' ? 'on' : ''}`} onClick={() => setTab('offers')}>Offers</button></>}</div>}
    {tab === 'applications' && <div className="card"><div className="tbl-scroll"><table className="tbl"><thead><tr><th>Applicant</th><th>Programme</th><th>Canonical status</th><th /></tr></thead><tbody>{apps.applications.map((a: any) => <tr key={a.id}><td><b>{a.name}</b><div className="hint">{a.email}</div></td><td>{a.program}</td><td><span className={`pill s-${String(a.current_status || a.status).toLowerCase()}`}>{a.current_status || a.status}</span></td><td>{!directorMode && a.current_status === 'SUBMITTED' && <button className="btn btn-sm btn-out" disabled={!caps.verify} onClick={() => act(() => api.decideApplication(a.id, 'verify'))}>Verify</button>}</td></tr>)}</tbody></table></div></div>}
    {tab === 'corrections' && <div className="card"><div className="card-pad"><h3>Corrections</h3><p className="hint">Applications requiring applicant correction or awaiting review after resubmission.</p><div className="tbl-scroll"><table className="tbl"><thead><tr><th>Applicant</th><th>Programme</th><th>Current status</th></tr></thead><tbody>{apps.applications.filter((a: any) => ['CORRECTION_REQUIRED', 'RESUBMITTED'].includes(a.current_status)).map((a: any) => <tr key={a.id}><td><b>{a.name}</b><div className="hint">{a.email}</div></td><td>{a.program}</td><td><span className={`pill s-${String(a.current_status).toLowerCase()}`}>{a.current_status}</span></td></tr>)}</tbody></table></div></div></div>}
    {tab === 'review' && <><div className="card" style={{ marginBottom: 12 }}><div className="card-pad"><input className="inp" placeholder="Search applicant or application no." value={search} onChange={e => setSearch(e.target.value)}/><button className="btn btn-out" onClick={loadQueue}>Apply</button></div></div><div className="card"><div className="tbl-scroll"><table className="tbl"><thead><tr><th>Application</th><th>Applicant</th><th>Cycle</th><th>Documents</th><th>Status</th></tr></thead><tbody>{queue.map(a => <tr key={a.id} style={{ cursor: 'pointer' }} onClick={() => api.admissionDetail(a.id).then(setDetail).catch(e => setNotice({ outcome: 'DENY', reason: e.message }))}><td className="mono">{a.application_no}</td><td>{a.applicant_name}</td><td>{a.cycle}</td><td>{a.documents.uploaded}/{a.documents.required}</td><td><span className={`pill s-${String(a.current_status).toLowerCase()}`}>{a.current_status}</span></td></tr>)}</tbody></table></div></div></>}
    {tab === 'eligibility' && <EligibilityQueue eligibility={eligibility} filters={filters} setFilters={setFilters} cycles={cycles} programmes={programmes} load={loadEligibility} detail={openEligibilityDetail} act={act} />}
    {tab === 'rules' && <ConfigList title="Eligibility Rules" items={rules} cycles={cycles} programmes={programmes} cycleId={filters.cycle_id} setCycleId={(v: string) => setFilters({ ...filters, cycle_id: v })} reload={loadRules} canManage={!!caps.manage_eligibility_rules} create={() => setRule({ ...emptyRule, cycle_id: filters.cycle_id })} edit={editRule} kind="rule" />}
    {tab === 'quotas' && <ConfigList title="Quotas" items={quotas} cycles={cycles} programmes={programmes} cycleId={filters.cycle_id} setCycleId={(v: string) => setFilters({ ...filters, cycle_id: v })} reload={loadQuotas} canManage={!!caps.manage_quotas} create={() => setQuota({ ...emptyQuota, cycle_id: filters.cycle_id })} edit={setQuota} kind="quota" />}
    {tab === 'decisions' && <div className="card"><div className="card-pad"><h3>Assessment, Merit & Allocation</h3><p className="hint">All scores, merit and seat availability are calculated by the backend.</p><div className="tbl-scroll"><table className="tbl"><thead><tr><th>Application</th><th>Status</th><th>Actions</th></tr></thead><tbody>{phase4.map(a => <tr key={a.id}><td>{a.name}<div className="hint">{a.program}</div></td><td>{a.current_status}</td><td>{a.current_status === 'ELIGIBLE' && <button className="btn btn-sm btn-out" onClick={() => act(() => api.advanceAdmissionPhase4(a.id,a.status_version),loadPhase4)}>Advance</button>}{a.current_status === 'ASSESSMENT_PENDING' && <button className="btn btn-sm btn-brass" onClick={() => setAssessment({ ...a, assessment_type: 'ENTRANCE_EXAM', score: '', max_score: '' })}>Record assessment</button>}{['ELIGIBLE','ASSESSMENT_PENDING','ASSESSMENT_QUALIFIED','COUNSELLING_PENDING','ALLOCATION_PENDING'].includes(a.current_status) && <button className="btn btn-sm btn-out" onClick={() => act(() => api.calculateAdmissionMerit(a.id),loadPhase4)}>Merit</button>}{a.current_status === 'ALLOCATION_PENDING' && seatPools.filter(p => p.program_id === a.program_id && p.available > 0).map(p => <button key={p.id} className="btn btn-sm btn-brass" onClick={() => act(() => api.allocateAdmissionSeat(a.id,{seat_pool_id:p.id,expected_status_version:a.status_version}),loadPhase4)}>Allocate</button>)}</td></tr>)}</tbody></table></div></div></div>}
    {tab === 'counselling' && <div className="card"><div className="card-pad"><h3>Counselling Queue</h3><div className="tbl-scroll"><table className="tbl"><thead><tr><th>Applicant</th><th>Programme</th><th>Merit</th><th>Preferences</th><th /></tr></thead><tbody>{counselling.map(a => <tr key={a.id}><td>{a.name}</td><td>{a.program}</td><td>{a.merit_score ?? 'Not calculated'} {a.merit_rank ? `· #${a.merit_rank}` : ''}</td><td>{a.preferences?.map((p:any) => `#${p.rank}`).join(', ') || '—'}</td><td><button className="btn btn-sm btn-out" onClick={() => setCounsellingItem({ ...a, attendance_status: 'attended', remarks: '', preference_rank: 1 })}>Record outcome</button></td></tr>)}</tbody></table></div></div></div>}
    {tab === 'seatpools' && <div className="card"><div className="card-pad"><h3>Seat Pools</h3><button className="btn btn-brass" onClick={() => setSeatPool({ cycle_id: filters.cycle_id, campus: '', program_id: '', quota_id: '', capacity: 0, status: 'open' })}>Configure seat pool</button><div className="tbl-scroll"><table className="tbl"><thead><tr><th>Campus</th><th>Programme</th><th>Quota</th><th>Capacity</th><th>Reserved / allocated</th><th>Available</th><th>Waitlisted</th><th>Status</th><th /></tr></thead><tbody>{seatPools.map(p => <tr key={p.id}><td>{p.campus}</td><td>{programmes.find((x:any) => x.id === p.program_id)?.name || p.program_id}</td><td>{p.quota_id || 'General'}</td><td>{p.capacity}</td><td>{p.used}</td><td>{p.available}</td><td>{p.waitlisted}</td><td>{p.status}</td><td><button className="btn btn-sm btn-out" onClick={() => setSeatPool({ ...p })}>Edit</button></td></tr>)}</tbody></table></div></div></div>}
    {tab === 'waitlist' && <div className="card"><div className="card-pad"><h3>Waitlist</h3><div className="tbl-scroll"><table className="tbl"><thead><tr><th>Application</th><th>Applicant</th><th>Pool</th><th>Merit rank</th><th>Position</th><th>Round</th><th>Status</th></tr></thead><tbody>{waitlist.map(x => <tr key={x.id}><td>{x.application_no}</td><td>{x.applicant_name}</td><td className="mono">{x.seat_pool_id}</td><td>{x.rank ?? '—'}</td><td>{x.position}</td><td>{x.round_no}</td><td>{x.status}</td></tr>)}</tbody></table></div></div></div>}
    {tab === 'offers' && <div className="card"><div className="card-pad"><h3>Offers</h3><select className="inp" value={offerStatus} onChange={e => { setOfferStatus(e.target.value); setTimeout(loadOffers, 0) }}><option value="">All offer states</option>{['OFFER_RECOMMENDATION_PENDING','OFFER_APPROVAL_PENDING','OFFERED','OFFER_ACCEPTED','OFFER_DECLINED','OFFER_EXPIRED'].map(x => <option key={x}>{x}</option>)}</select><div className="tbl-scroll"><table className="tbl"><thead><tr><th>Applicant</th><th>Programme</th><th>State</th><th>Workflow</th><th>Offer</th><th /></tr></thead><tbody>{offers.map(a => <tr key={a.id}><td>{a.name}</td><td>{a.program}</td><td>{a.current_status}</td><td>{a.workflow_state || '—'}</td><td>{a.offer_no || 'Not issued'}<div className="hint">{a.expires_at?.slice(0,10)}</div></td><td>{a.can_recommend && <button className="btn btn-sm btn-out" onClick={() => act(() => api.recommendAdmissionOffer(a.id,a.status_version),loadOffers)}>Recommend</button>}{a.can_issue && <button className="btn btn-sm btn-brass" onClick={() => act(() => api.issueAdmissionOffer(a.id,a.status_version),loadOffers)}>Issue</button>}</td></tr>)}</tbody></table></div></div></div>}
    {tab === 'program_intake' && <ProgramIntake rows={programIntake}/>} {tab === 'document_status' && <DocumentStatus rows={documentStatus}/>} {tab === 'final_approval' && <FinalApprovalStatus rows={finalApprovals}/>} {tab === 'reports' && <AdmissionsReports apps={apps.applications} phase5={phase5Status}/>} {tab === 'recommendations' && <OfferStatus rows={offers.filter(x => ['OFFER_RECOMMENDATION_PENDING','OFFER_APPROVAL_PENDING'].includes(x.current_status))} title="Offer Recommendations"/>} {tab === 'issued_offers' && <OfferStatus rows={offers.filter(x => ['OFFERED','OFFER_ACCEPTED','OFFER_DECLINED','OFFER_EXPIRED'].includes(x.current_status))} title="Issued Offers"/>} {tab.startsWith('director_') && <DirectorMonitoring mode={tab} apps={apps.applications} data={directorData}/>} {['finance_status','invoices_challans','payment_status','accounts_verification','clearance_status'].includes(tab) && <ApplicantFinanceView rows={phase5Status} mode={tab}/>} {['ready_to_admit','enrollment_queue','student_conversion','enrollment_status'].includes(tab) && <FinalAdmissionView rows={phase5Status} mode={tab}/>}
    {tab === 'cycles' && <Cycles cycles={cycles} programmes={programmes} manage={manage} setCycle={setCycle} binding={binding} setBinding={setBinding} act={act} loadCycles={loadCycles} />}
    {cycle && <CycleModal cycle={cycle} setCycle={setCycle} binding={binding} setBinding={setBinding} programmes={programmes} save={saveCycle} act={act} loadCycles={loadCycles} />}
    {rule && <RuleModal rule={rule} setRule={setRule} cycles={cycles} programmes={programmes} quotas={quotas} save={saveRule} />}
    {quota && <QuotaModal quota={quota} setQuota={setQuota} cycles={cycles} programmes={programmes} save={saveQuota} />}
    {assessment && <Modal title={`Assessment: ${assessment.name}`} onClose={() => setAssessment(null)} footer={<><button className="btn btn-out" onClick={() => setAssessment(null)}>Cancel</button><button className="btn btn-brass" onClick={() => act(() => api.recordAdmissionAssessment(assessment.id,{assessment_type:assessment.assessment_type,score:Number(assessment.score),max_score:Number(assessment.max_score),expected_status_version:assessment.status_version}),() => { setAssessment(null); loadPhase4() })}>Verify result</button></>}><select className="inp" value={assessment.assessment_type} onChange={e => setAssessment({ ...assessment, assessment_type: e.target.value })}>{['ENTRANCE_EXAM','ACADEMIC_MERIT','OTHER'].map(x => <option key={x}>{x}</option>)}</select><input className="inp" type="number" placeholder="Verified score" value={assessment.score} onChange={e => setAssessment({ ...assessment, score: e.target.value })}/><input className="inp" type="number" placeholder="Maximum score" value={assessment.max_score} onChange={e => setAssessment({ ...assessment, max_score: e.target.value })}/></Modal>}
    {seatPool && <Modal title="Seat pool" onClose={() => setSeatPool(null)} footer={<><button className="btn btn-out" onClick={() => setSeatPool(null)}>Cancel</button><button className="btn btn-brass" onClick={() => act(() => api.createAdmissionSeatPool({ ...seatPool, quota_id: seatPool.quota_id || null, capacity: Number(seatPool.capacity), category_code: '', intake_key: '' }),() => { setSeatPool(null); loadPhase4() })}>Save</button></>}><select className="inp" value={seatPool.cycle_id} onChange={e => setSeatPool({ ...seatPool, cycle_id: e.target.value })}><option value="">Cycle</option>{cycles.map((c:any) => <option key={c.id} value={c.id}>{c.name}</option>)}</select><input className="inp" placeholder="Campus" value={seatPool.campus} onChange={e => setSeatPool({ ...seatPool, campus: e.target.value })}/><select className="inp" value={seatPool.program_id} onChange={e => setSeatPool({ ...seatPool, program_id: e.target.value })}><option value="">Programme</option>{programmes.map((p:any) => <option key={p.id} value={p.id}>{p.name}</option>)}</select><select className="inp" value={seatPool.quota_id || ''} onChange={e => setSeatPool({ ...seatPool, quota_id: e.target.value })}><option value="">General quota</option>{quotas.map((q:any) => <option key={q.id} value={q.id}>{q.name}</option>)}</select><input className="inp" type="number" placeholder="Capacity" value={seatPool.capacity} onChange={e => setSeatPool({ ...seatPool, capacity: e.target.value })}/><select className="inp" value={seatPool.status} onChange={e => setSeatPool({ ...seatPool, status: e.target.value })}><option value="open">Active</option><option value="closed">Inactive</option></select></Modal>}
    {counsellingItem && <Modal title={`Counselling: ${counsellingItem.name}`} onClose={() => setCounsellingItem(null)} footer={<><button className="btn btn-out" onClick={() => setCounsellingItem(null)}>Cancel</button><button className="btn btn-brass" onClick={() => act(() => api.recordCounselling(counsellingItem.id,{attendance_status:counsellingItem.attendance_status,recommended_program_id:counsellingItem.program_id,preference_rank:Number(counsellingItem.preference_rank),remarks:counsellingItem.remarks,expected_status_version:counsellingItem.status_version}),() => { setCounsellingItem(null); loadCounselling() })}>Complete counselling</button></>}><p>Merit: {counsellingItem.merit_score ?? 'Not calculated'} · rank {counsellingItem.merit_rank ?? '—'}</p><select className="inp" value={counsellingItem.attendance_status} onChange={e => setCounsellingItem({ ...counsellingItem, attendance_status: e.target.value })}><option value="attended">Attended</option><option value="absent">Absent</option></select><input className="inp" type="number" value={counsellingItem.preference_rank} onChange={e => setCounsellingItem({ ...counsellingItem, preference_rank: e.target.value })}/><input className="inp" placeholder="Remarks" value={counsellingItem.remarks} onChange={e => setCounsellingItem({ ...counsellingItem, remarks: e.target.value })}/></Modal>}
    {detail && <Modal title={detail.application_no} onClose={() => setDetail(null)} footer={<button className="btn btn-out" onClick={() => setDetail(null)}>Close</button>}><p>{detail.applicant_name} · {detail.email}</p><p>{detail.current_status} · v{detail.status_version}</p></Modal>}
    {eligibilityDetail && <EligibilityDetail detail={eligibilityDetail} onClose={() => setEligibilityDetail(null)} />}
