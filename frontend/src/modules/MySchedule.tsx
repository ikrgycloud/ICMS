...
export default function MySchedule({ user, go }: { user: any; go: (view: string) => void }) {
  ...
  return <div className="schedule-page fade-in">
    <header className="schedule-head"><div className="schedule-head-copy"><h1>My Schedule</h1><p>Your personal schedule of meetings, deadlines, reviews, hearings, and events.</p><div className="schedule-context"><span className="schedule-scope">{user.scope || 'Campus'}</span><span>{data.range?.label || 'Current month'}</span><span>Acting as <b>{user.active_role}</b></span></div></div><button className="schedule-add" onClick={() => go('calendar')} type="button">+ Add Event</button></header>
