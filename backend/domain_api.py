...
def _faculty_feature_flags(s, ctx):
    ...
    advisor = "faculty_advisor" in roles
...
def workspace(ctx=Depends(auth), s=Depends(db)):
    ...
    o = office(ctx["office_n"])
    mods = []
    features = _faculty_feature_flags(s, ctx) if ctx["office_n"] in {11, 12, 13, 14} else {}
    for key in modules_for_office(ctx["office_n"]):
        if key == "research" and ctx["office_n"] in {11, 12, 13, 14} and not features.get("research"):
            continue
    ...
    return {"office_n": ctx["office_n"], "office": o.get("name"),
            ...
            "modules": mods, "faculty_features": features}
...
def research(ctx=Depends(auth), s=Depends(db)):
    ...
    assignments = _active_faculty_assignments(s, ctx) if ctx["office_n"] in {11, 12, 13, 14} else []
    research_roles = {"research_faculty", "pi", "principal_investigator", "co_pi", "research_supervisor"}
    assignments = [row for row in assignments if row.role_key.lower() in research_roles]
    if ctx["office_n"] in {11, 12, 13, 14} and not assignments:
        raise HTTPException(403, "An active research responsibility is required")
    refs = {row.scope_ref for row in assignments if row.scope_type == "research_project" and row.scope_ref}
    staff = _staff_profile(s, ctx)
    rows = s.query(D.ResearchProject).all()
    if ctx["office_n"] in {11, 12, 13, 14}:
        staff_names = {staff.name.lower()} if staff else set()
        rows = [row for row in rows if row.id in refs or (row.pi_name or "").lower() in staff_names]
    total = s.query(func.coalesce(func.sum(D.ResearchProject.grant_amount), 0)).scalar() or 0
    if ctx["office_n"] in {11, 12, 13, 14}:
        total = sum(row.grant_amount or 0 for row in rows)
