...
class Phase5CRiskTests(unittest.TestCase):
    ...
    def test_invalid_risk_transition_and_high_risk_escalation(self):
        ...
        self.assertEqual(escalated["destination"], "Principal")
        self.assertEqual(escalated["risk"]["escalation_destination"], "Principal")
        ...
        notifications = self.session.query(Notification).filter(Notification.title.like("Approval required%"), Notification.body.like(f"%{risk['title']}%")).all()
        self.assertTrue(any(row.user_id == "user_4" for row in notifications))
