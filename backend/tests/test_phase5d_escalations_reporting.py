...
class Phase5DEscalationReportTests(unittest.TestCase):
    ...
    def test_campus_head_receive_scope_and_routing_regressions(self):
        main_campus = self.session.get(OrgScope, "scope_main_campus")
        if main_campus is None:
            self.skipTest("Main campus scope seed missing")
        self.session.add(D.Student(id=f"student_scope_main_{datetime.utcnow().strftime('%H%M%S%f')}", tenant_id=TENANT, roll_no="SMAIN01", name="Main Campus Student", email="main@example.com", dept_id=None, program_id=None, campus="Main Campus", campus_scope_id="scope_main_campus", batch="2025", semester=1, section="A", status="active", cgpa=8.5))
        self.session.add(D.Student(id=f"student_scope_north_{datetime.utcnow().strftime('%H%M%S%f')}", tenant_id=TENANT, roll_no="SNORTH01", name="North Campus Student", email="north@example.com", dept_id=None, program_id=None, campus="North Campus", campus_scope_id="scope_north_campus", batch="2025", semester=1, section="A", status="active", cgpa=7.5))
        self.session.commit()
