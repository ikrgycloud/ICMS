...
class Phase5ERegressionTests(unittest.TestCase):
    ...
    def test_canonical_student_scope(self):
        stamp = datetime.utcnow().strftime("%H%M%S%f")
        main_student = D.Student(id=f"phase5e-main-{stamp}", tenant_id=TENANT, roll_no=f"P5EM{stamp[-6:]}", name="Phase5E Main", email=f"main-{stamp}@example.test", campus="Main Campus", campus_scope_id="scope_main_campus", batch="2025", semester=1, section="A", status="active", cgpa=8.0)
        north_student = D.Student(id=f"phase5e-north-{stamp}", tenant_id=TENANT, roll_no=f"P5EN{stamp[-6:]}", name="Phase5E North", email=f"north-{stamp}@example.test", campus="North Campus", campus_scope_id="scope_north_campus", batch="2025", semester=1, section="A", status="active", cgpa=8.0)
        ...
        students = domain_api.list_students(q="Phase5E Main", ctx=self.campus_ctx, s=self.session)["students"]
