...
class CourseMaterial(Base):
    """A professor-managed resource published to one teaching section."""
...
class StaffCheckIn(Base):
    """Daily faculty presence record, kept separate from student attendance."""
...
class MentorAssignment(Base):
    """Formal faculty-to-student advising assignment."""
...
class FacultyFunctionalAssignment(Base):
    """Time-bound faculty responsibility used for portal access and scope."""
    __tablename__ = "faculty_functional_assignments"
    id = Column(String, primary_key=True)
    tenant_id = Column(String, index=True)
    faculty_id = Column(String, ForeignKey("staff_members.id"), index=True)
    role_key = Column(String, index=True)  # course_coordinator/faculty_advisor/research_faculty/pi/co_pi/research_supervisor
    permission_key = Column(String, default="", index=True)
    scope_type = Column(String, default="")  # section/course/department/research_project
