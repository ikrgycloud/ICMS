...
def _ensure_student_portal_demo_sections(s, dept_id: str):
    ...
    for spec in STUDENT_PORTAL_ATTENDANCE_DEMO:
        ...
        faculty.campus_scope_id = faculty.campus_scope_id or "scope_main_campus"
...
def _seed_core_domain(s):
    if s.query(D.Student).count() > 0 and s.query(D.Course).count() > 0:
        return
    ...
    for cat, alloc in [
        ...
        s.add(D.BudgetLine(id=f"bud_{cat.lower().replace(' ', '_')}", tenant_id=TENANT,
                           campus=CAMPUS_SCOPES[0], campus_scope_id="scope_main_campus", category=cat, allocated=alloc,
    ...
    for i in range(1, 11):
        ...
        s.add(D.LeaveRequest(id=f"leave_{i}", tenant_id=TENANT, staff_id=fid,
                             staff_name=fnm, campus_scope_id="scope_main_campus", kind=R.choice(["Casual", "Medical", "Earned"]),
    ...
    for block in ["A-Block", "B-Block", "C-Block"]:
        for rn in range(101, 121):
            ...
            s.add(D.HostelRoom(id=rid, tenant_id=TENANT, campus=CAMPUS_SCOPES[0], block=block,
                               campus_scope_id="scope_main_campus", room_no=str(rn), capacity=2, occupied=R.randint(0, 2)))
    s.flush()
    for i in range(1, 16):
        s.add(D.HostelAllocation(id=f"halloc_{i}", tenant_id=TENANT, campus=CAMPUS_SCOPES[0], campus_scope_id="scope_main_campus",
    ...
    for i, (name, stops) in enumerate([
        ...
        s.add(D.TransportRoute(id=f"route_{i}", tenant_id=TENANT, campus_scope_id="scope_main_campus", name=name, stops=stops,
    ...
    for i in range(1, 13):
        ...
        s.add(D.ResearchProject(id=f"proj_{i}", tenant_id=TENANT, campus_scope_id="scope_main_campus",
                                title=R.choice([
...
def _bind_portal_accounts(s):
    ...
    if stu_login and "CSE" in dept_ids:
        ...
        if stu_login.scope_ref and not str(stu_login.scope_ref).startswith("scope_"):
            pick = s.get(D.Student, stu_login.scope_ref)
        ...
        if pick:
            ...
            if any_room:
                _ensure(
                    ...
                    lambda pick=pick, any_room=any_room: D.HostelAllocation(
                        ...
                        campus_scope_id=pick.campus_scope_id,
            ...
            program = s.get(D.Program, pick.program_id) if pick.program_id else None
            if demo_sections:
                _ensure(
...
def _seed_student_portal_demo_profile(s):
    ...
    student = None
    if stu_login.scope_ref and not str(stu_login.scope_ref).startswith("scope_"):
        student = s.get(D.Student, stu_login.scope_ref)
    if not student:
        student = s.query(D.Student).filter(D.Student.user_id == stu_login.id).order_by(D.Student.roll_no).first()
    if not student:
        return
    # All records below use deterministic primary keys and `_ensure`, so a
    # startup is an upsert rather than a delete-and-recreate cycle.  Removing
    # the historical rows first was unsafe once assessments were referenced by
    # the examination lifecycle and could make a development restart fail on a
    # duplicate key after a partial seed transaction.  Existing demo history
    # is therefore preserved and refreshed in place.
    ...
    for req_id, reference, title, category, department, priority, due_date, evidence, workflow_id in records:
        _ensure(s, WorkflowInstance, workflow_id,
                lambda workflow_id=workflow_id, title=title: WorkflowInstance(
                    ...
                    initiator_name="Dean R&D / IQAC", current_stage=2, scope_level="campus",
                    campus_scope_id="scope_main_campus", escalated=False))
    ...
    for req_id, reference, title, category, department, priority, due_date, evidence, workflow_id in records:
        _ensure(s, D.ComplianceRequirement, req_id,
                ...
                workflow_id=workflow_id: D.ComplianceRequirement(
                    id=req_id, tenant_id=TENANT, campus="Main Campus", campus_scope_id="scope_main_campus", reference_code=reference,
