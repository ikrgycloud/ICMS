...
def _bind_portal_accounts(s):
    ...
    stu_login = _user(DEMO_USERNAMES[36])
    if stu_login and "ECE" in dept_ids:
        existing_bound = s.query(D.Student).filter(D.Student.user_id == stu_login.id).order_by(D.Student.roll_no).all()
        # Always bind this login to the ECE record with the requested roll
        # number.  Do not reuse a prior scope_ref: older seeds linked the
        # generic student account to a CSE record, which made the professor
        # roster and the student portal refer to different students.
        pick = (s.query(D.Student)
                .filter(D.Student.dept_id == dept_ids["ECE"], D.Student.roll_no == "25ECE072")
                .first())
        if not pick:
            pick = (s.query(D.Student)
                    .filter(D.Student.dept_id == dept_ids["ECE"], D.Student.batch == "2025")
                    .order_by(D.Student.cgpa.desc()).first())
        if not pick:
            pick = (s.query(D.Student)
                    .filter(D.Student.dept_id == dept_ids["ECE"])
                    .order_by(D.Student.cgpa.desc()).first())
        if pick:
            demo_sections = _ensure_student_portal_demo_sections(s, dept_ids["ECE"])
            for row in existing_bound:
                if row.id != pick.id:
                    row.user_id = None
            pick.user_id = stu_login.id
            pick.roll_no = "25ECE072"
            pick.name = "Nisha Mehta"
            pick.email = "25ece072@icms.edu"
            pick.semester = 3
            pick.section = "A"
            pick.batch = "2025"
            pick.status = "active"
    ...
    # Make the named student login demonstrable end-to-end: the Professor
    # account teaches Nisha's active portal sections, so marks entered from
    # that account target the same student record seen by her portal.
    professor_login = _user(DEMO_USERNAMES[11])
    nisha = (s.query(D.Student)
             .filter(D.Student.user_id == stu_login.id)
             .first()) if stu_login else None
    professor_staff = (s.query(D.StaffMember)
                       .filter(D.StaffMember.user_id == professor_login.id)
                       .first()) if professor_login else None
    if nisha and professor_staff:
        nisha_section_ids = [row.section_id for row in s.query(D.Enrollment)
                             .filter(D.Enrollment.student_id == nisha.id,
                                     D.Enrollment.status == "enrolled")
                             .all()]
        if nisha_section_ids:
            (s.query(D.Section)
             .filter(D.Section.id.in_(nisha_section_ids))
             .update({"faculty_person_id": professor_staff.id}, synchronize_session=False))
...
def _seed_development_backlog_history(s):
    ...
    portal_login = s.query(User).filter(User.username == DEMO_USERNAMES[36]).first()
    if portal_login:
...
def _seed_student_portal_demo_profile(s):
    stu_login = s.query(User).filter(User.username == DEMO_USERNAMES[36]).first()
    if not stu_login:
        return
    ...
    # Marks may be entered or edited outside this demo seeder, so their IDs do
    # not necessarily use the demo prefix.  Delete every mark linked to the
    # archived assessments before rebuilding those assessments.
    s.query(D.Mark).filter(D.Mark.assessment_id.like(f"asmt_scorehist_{student.id}_%")).delete(synchronize_session=False)
